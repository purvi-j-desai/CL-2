...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.7.9l 2012-04-25 12:57:55 GMT")
...
{
	(void)soap; (void)a; /* appease -Wall -Werror */
	soap_serialize_PointerTodouble(soap, &a->res);
}
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_ns__addResponse(struct soap *soap, const char *tag, int id, const struct ns__addResponse *a, const char *type)
{
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_ns__addResponse), type))
		return soap->error;
	if (soap_out_PointerTodouble(soap, "res", -1, &a->res, ""))
		return soap->error;
	return soap_element_end_out(soap, tag);
}
	...
	if (soap->body && !*soap->href)
	{
		...
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_res && soap->error == SOAP_TAG_MISMATCH)
				if (soap_in_PointerTodouble(soap, "res", &a->res, "xsd:double"))
				{	soap_flag_res--;
					continue;
				}
			if (soap->error == SOAP_TAG_MISMATCH)
